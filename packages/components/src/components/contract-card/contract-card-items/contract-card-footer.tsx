...
const CardFooter = ({
            ...
            {is_multiplier ? (
                ...
                <React.Fragment>
                    ...
                    {should_show_sell_note && (
                        <Text as='div' size='xxxs' color='less-prominent' line_height='s' className='lookbacks--note'>
                            <Localize
    ...
    // If transitions are disabled, render directly
    if (!should_show_transition) {
        return footer_content;
    }

    // Use CSSTransition only when transitions are enabled
    return (
        <CSSTransition
            in={in_prop}
            timeout={250}
